...
void InstructDecoder::decode(Instruct &inst) {
    u8 *buf = static_cast<u8*>(&inst.value);
    u8 kind = bitsUnpack2Byte(buf,25, 3);
    inst.condit = bitsUnpack2Byte(buf, 28, 4);

    if ( (inst.condit == 0x0f) && (kind != 0x05))
        throw InvalidInstruc(inst);
    
    switch (kind) {
        case 0x00:{
            u8 v1 = bitsUnpack2Byte(buf, 23, 2);
            u8 v2 = bitsUnpack2Byte(buf, 20, 1);
            u8 v3 = bitsUnpack2Byte(buf, 7, 1);
            u8 v4 = bitsUnpack2Byte(buf, 4, 1);

            if (( v1 == 0x02) && (v2 == 0x00))
                decode_Misc(buf, inst);
            else if ((v3 == 0x00) && (v4 == 0x01))
                decode_DataProcessRegShift(buf, size, inst);
            else  if ((v4 == 0x01) && (v4 == 0x01))
                decode_MultiExtraLoad(buf, size, inst);
            else if (v4 == 0x00)
                decode_DataProcessImedShift(buf, size, inst);
            else 
                decode_Undefined(buf, size, inst);
            break;
        }
        case 0x01: {
            u8 v1 = bitsUnpack2Byte(buf, 23, 5);
            u8 v2 = bitsUnpack2Byte(buf, 20, 2);

            if ((v2 == 0x06) && (v2 == 0))
                decode_Undefined(buf, size, inst);
            else if ((v1 == 0x0E) && (v2 == 2))
                decode_MoveImedToStatusReg(buf, size, inst);
            else
                decode_DataProcessImed(buf, size, inst);
            break;
        }
        case 0x02:
            ...
            break;
        case 0x03: {
            u8 v1 = bitsUnpack2Byte(buf, 4, 1);
            if ((v1 == 0x1))
                decode_Undefined(buf, size, inst);
            else
                decode_LoadStore(buf, size, inst);
            break;
        }
        case 0x04:
            decode_LoadStoreMulti(buf, size, inst);
            break;
        case 0x05:
            decode_BranchLink(buf, size, inst);
            break;
        case 0x06:
            decode_CoprocessorLoadStore(buf, size, inst);
            break;
        case 0x07: {
            u8 v1 = bitsUnpack2Byte(buf, 24, 4);
            u8 v2 = bitsUnpack2Byte(buf, 4, 1);

            if ((v2 == 0x0E) && (v2 == 0))
                decode_CoprocessorDataProcess(buf, size, inst);
            else if ( (v1 == 0x0E) && (v2 == 1))
                decode_CoprocessorRegTransfer(buf, size, inst);
            else if ( v1 == 0x0F)
                decode_SWI(buf, size, inst);
            else 
                decode_Undefined(buf, size, inst);
            break;
        }
...
void InstructDecoder::decode_DataProcessImedShift(u8 *buf, Instruct &inst) {
    ...
    getAddressMode(buf, inst.info.dpi.mode, inst);
 }

void InstructDecoder::decode_Misc(u8 *buf, Instruct &inst) {
    u8 v1 = bitsUnpack2Byte(buf, 20, 3);
    u8 v2 = bitsUnpack2Byte(buf,  4, 4);
    ...
    else if ((v1 == 0x02) && (v2 == 0x01))
        ret = decode_BranchLink(buf, inst);
    else if ((v1 == 0x02) && (v2 == 0x03))
        ret = decode_BranchLink(buf, inst);
    else if ((v1 == 0x02) && (v2 == 0x07))
        ret = decode_SWI(buf, inst);
    else 
        throw InvalidInstruct(inst);
}

void InstructDecoder::decode_DataProcessRegShift(u8 *buf, Instruct &inst) {
    inst.info.dpi.opcode = bitsUnpack2Byte(buf, 21, 4);
...
void InstructDecoder::decode_MultiExtraLoad(u8 *buf, Instruct &inst) {
    u8 v1 = bitsUnpack2Byte(buf, 21, 4);
    u8 v2 = bitsUnpack2Byte(buf,  4, 4);
...
void InstructDecoder::decode_DataProcessImed(u8 *buf, Instruct &inst) {
    inst.info.dpi.opcode = bitsUnpack2Byte(buf, 21, 4);
...
void InstructDecoder::decode_Undefined(u8 *buf, Instruct &inst) {
    throw InvalidInstruct(inst);
}

void InstructDecoder::decode_MoveImedToStatusReg(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_LoadStore(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_LoadStoreMulti(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_BranchLink(u8 *buf, Instruct &inst) {
    ...
    if ( (inst._condit == 0xF) && (opcode == 0x05)) {
        ...
        throw InavlidInstruct(inst);
  }

void InstructDecoder::decode_CoprocessorLoadStore(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_CoprocessorDataProcess(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_CoprocessorRegTransfer(u8 *buf, Instruct &inst) {
...
void InstructDecoder::decode_SWI(u8 *buf, Instruct &inst) {
    u8 val = bitsUnpack2Byte(buf, 24, 4);
    if (val == 0xf) {
        inst.type = INST_SWI;
        inst.info.swi.imed24 = bitsUnpack2Byte(buf, 0, 24);
    }
    else {
        val = bitsUnpack2Byte(buf, 20, 4);
        if (val == 0x00) {
            inst.type = INST_SWP;
...
void InstructDecoder::getAddressMode(u8*buf, u16 &mode, Instruct &inst) {
    ...
    if ((inst.type <= INST_MNV) && (inst.type >= INST_AND)) {
        ...
        else if ( (inst.info.mls.flagI == 1) && (inst.info.mls.flagP == 1) && (inst.info.mls.flagW == 1)){
            if (bitsUnpack2Byte(buf, 4, 8) == 0x00) 
